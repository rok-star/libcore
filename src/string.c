...
static char* __convert_case(char const* pchar, int len, int a, int b) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
...
static bool __all_included(char const* pchar, int len, uint32_t const* array, int size) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
	if (len > 0) {
		uint32_t codepoint = 0;
...
char* _string_clone(char const* pchar, int len) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
...
char* _string_uppercase(char const* pchar, int len) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
	return __convert_case(pchar, len, 0, 1);
}

char* _string_lowercase(char const* pchar, int len) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
	return __convert_case(pchar, len, 1, 0);
}

bool _string_compare(char const* pchar1, char const* pchar2, int len) {
	_ASSERT(pchar1 != NULL);
	_ASSERT(pchar2 != NULL);
	_ASSERT(len >= 0);
	if (len > 0) {
...
void _string_tokenize(char const* pchar, int len, char*** tokens, int* size) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
	_ASSERT(tokens != NULL);
	_ASSERT(size >= 0);
...
void _string_split(char const* pchar, int len, char const* separator, char const*** array, int* size) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
...
bool _string_whitespace(char const* pchar, int len) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
	return __all_included(pchar, len, __whitespace, 25);
}

bool _string_digital(char const* pchar, int len) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
