...
static char* __convert_case(char const* pchar, int64_t len, int a, int b) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
	int64_t read = 0;
	int64_t lindex = 0;
...
static bool __all_included(char const* pchar, int64_t len, uint32_t const* array, int64_t size) {
	_ASSERT(pchar != NULL);
	_ASSERT(len >= 0);
	if (len > 0) {
		uint32_t codepoint = 0;
