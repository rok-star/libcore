...
static void __pass_char(char const* pchar, int64_t len, char ch, int* read) {
	_ASSERT(pchar != NULL);
	_ASSERT(read != NULL);
...
static void __read_sign(char const* pchar, int64_t len, int* sign, int* read) {
	_ASSERT(pchar != NULL);
	_ASSERT(sign != NULL);
	_ASSERT(read != NULL);
...
static void __read_digits(const char* pchar, int64_t len, bool spaces, int** digits, int* num, int* read) {
	_ASSERT(pchar != NULL);
	_ASSERT(digits != NULL);
...
static bool __build_number(int* digits, int size, uint64_t* out) {
	_ASSERT(digits != NULL);
	_ASSERT(size > 0);
	_ASSERT(size <= 20);
	...
	for (int i = (size - 1); i >= 0; i--) {
		val += digits[i] * (__uint128_t)pow(10, exp);
		exp += 1;
	}
...
static int64_t __read_number(char const* pchar, int64_t len, bool read_space, bool read_sign, bool read_prec, int* out_sign, uint64_t* out_left, uint64_t* out_right) {
	_ASSERT(pchar != NULL);
	_ASSERT(out_left != NULL);
	_ASSERT((read_prec && (out_right != NULL)) || !read_prec);
	_ASSERT((read_sign && (out_sign != NULL)) || !read_sign);
	...
	if (len > 0) {
		...
		if (read_sign) {
			__read_sign(pchar, len, &sign, &read);
			if (read > 0) {
				total += read;
				pchar += read;
				len -= read;
		...
		__read_digits(pchar, len, read_space, &digits, &num, &read);
		if (read > 0) {
			total += read;
			pchar += read;
			len -= read;
			...
			if (__build_number(digits, num, &left)) {
				_FREE(digits);

				if (read_prec) {
					__pass_char(pchar, len, '.', &read);
					if (read > 0) {
						total += read;
						pchar += read;
						len -= read;
						...
						if (read > 0) {
							...
							if (__build_number(digits, num, &right)) {
								_FREE(digits);
								if (read_sign) {
									_ASSERT(out_sign != NULL);
									(*out_sign) = sign;
								}
								_ASSERT(out_right != NULL);
								(*out_left) = left;
								(*out_right) = right;
								return total;
							} else {
...
#define __READ_INT(pchar, len, out, spaces, sign, T, MAX) ({ \
	...
	uint64_t __value = 0; \
	int64_t __ret = 0; \
	int64_t __read = __read_number(pchar, len, spaces, sign, false, &__sign, &__value, NULL); \
	if (__read > 0) { \
		if (__value <= (uint64_t)MAX) { \
			if (sign) { \
				(*out) = (((T)__value) * __sign); \
			} else { \
				(*out) = (T)__value; \
			} \
			__ret = __read; \
...
int64_t _string_read_int8(char const* pchar, int64_t len, bool spaces, int8_t* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	return __READ_INT(pchar, len, out, spaces, true, int8_t, INT8_MAX);
}

int64_t _string_read_int16(char const* pchar, int64_t len, bool spaces, int16_t* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	return __READ_INT(pchar, len, out, spaces, true, int16_t, INT16_MAX);
}

int64_t _string_read_int32(char const* pchar, int64_t len, bool spaces, int32_t* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	return __READ_INT(pchar, len, out, spaces, true, int32_t, INT32_MAX);
}

int64_t _string_read_int64(char const* pchar, int64_t len, bool spaces, int64_t* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	return __READ_INT(pchar, len, out, spaces, true, int64_t, INT64_MAX);
}

int64_t _string_read_uint8(char const* pchar, int64_t len, bool spaces, uint8_t* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	return __READ_INT(pchar, len, out, spaces, false, uint8_t, UINT8_MAX);
}

int64_t _string_read_uint16(char const* pchar, int64_t len, bool spaces, uint16_t* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	return __READ_INT(pchar, len, out, spaces, false, uint16_t, UINT16_MAX);
}

int64_t _string_read_uint32(char const* pchar, int64_t len, bool spaces, uint32_t* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	return __READ_INT(pchar, len, out, spaces, false, uint32_t, UINT32_MAX);
}

int64_t _string_read_uint64(char const* pchar, int64_t len, bool spaces, uint64_t* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	return __READ_INT(pchar, len, out, spaces, false, uint64_t, UINT64_MAX);
}

int64_t _string_read_double(char const* pchar, int64_t len, bool spaces, double* out) {
	_ASSERT(pchar != NULL);
	_ASSERT(out != NULL);
	int sign = 1;
	uint64_t left = 0;
	uint64_t right = 0;
	int64_t read = __read_number(pchar, len, spaces, true, true, &sign, &left, &right);
	if (read > 0) {
		_ABORT("Not implemented\n");
	} else {
		return 0;
