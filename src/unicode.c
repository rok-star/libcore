...
int _codepoint_write_utf16(char32_t codepoint, char16_t* utf16_string, int capacity) {
	DS_ASSERT(utf16_string != NULL);
	if (capacity == 0) {
		return 0;
	} else if (codepoint >= 0xD800 && codepoint <= 0xDFFF)
		return 0;
		...
		if (capacity < 2)
			return 0;
		utf16_string[0] = (char16_t)codepoint;
		return 1;
	} else if (codepoint <= 0x10FFFF) {
		if (capacity < 3)
			return 0;
		codepoint &= 0xFFFF;
		utf16_string[0] = (char16_t)((codepoint >> 10) | 0xD800);
		utf16_string[1] = (codepoint & 0x3FF) | 0xDC00;
		return 2;
	} else return 0;
}

int _utf8_read_codepoint(char const* utf8_string, int length, char32_t* codepoint) {
	DS_ASSERT(utf8_string != NULL);
	DS_ASSERT(codepoint != NULL);
	(*codepoint) = 0;
	...
	if (utf8_string[0] == 0x0) {
		...
		if (length < 1)
			return 0;
		(*codepoint) = (uint8_t)utf8_string[0];
		read = 1;
	} else if ((uint8_t)utf8_string[0] <= 0xDF) {
		if (length < 2)
			return 0;
		(*codepoint) |= ((uint8_t)utf8_string[0] & 037) << 6;
		(*codepoint) |= ((uint8_t)utf8_string[1] & 077);
		read = 2;
	} else if ((uint8_t)utf8_string[0] <= 0xEF) {
		if (length < 3)
			return 0;
		(*codepoint) |= ((uint8_t)utf8_string[0] & 017) << 12;
		(*codepoint) |= ((uint8_t)utf8_string[1] & 077) << 6;
		(*codepoint) |= ((uint8_t)utf8_string[2] & 077);
		read = 3;
	} else if ((uint8_t)utf8_string[0] <= 0xF7) {
		if (length < 4)
			return 0;
		(*codepoint) |= ((uint8_t)utf8_string[0] & 007) << 18;
		(*codepoint) |= ((uint8_t)utf8_string[1] & 077) << 12;
		(*codepoint) |= ((uint8_t)utf8_string[2] & 077) << 6;
		(*codepoint) |= ((uint8_t)utf8_string[3] & 077);
		read = 4;
	}
...
int _utf16_read_codepoint(char16_t const* utf16_string, int length, char32_t* codepoint) {
	DS_ASSERT(utf16_string != NULL);
	DS_ASSERT(codepoint != NULL);
	int read = 0;
	if (utf16_string[0] < 0xD800 || utf16_string[0] > 0xDFFF) {
		if (length < 1)
			return 0;
		(*codepoint) = utf16_string[0];
		read = 1;
	} else {
		if (length < 2)
			return 0;
		(*codepoint) |= (utf16_string[0] & 0x3FF) << 10;
		(*codepoint) |= utf16_string[1] & 0x3FF;
		(*codepoint) |= 0x10000;
		read = 2;
	}
	if ((*codepoint) > 0x10FFFF) read = 0;
	else if ((*codepoint) >= 0xD800 && (*codepoint) <= 0xDFFF) read = 0;
	return read;
}

void __utf8_to_utf16(char const* utf8_string, int utf8_length, int* utf8_read, char16_t* utf16_string, int utf16_capacity, int* utf16_write) {
	DS_ASSERT(utf8_string != NULL);
	DS_ASSERT(utf8_read != NULL);
	DS_ASSERT(utf16_string != NULL);
	DS_ASSERT(utf16_write != NULL);
...
void __utf8_to_utf32(char const* utf8_string, int utf8_length, int* utf8_read, char32_t* utf32_string, int utf32_capacity, int* utf32_write) {
	DS_ASSERT(utf8_string != NULL);
	DS_ASSERT(utf8_read != NULL);
	DS_ASSERT(utf32_string != NULL);
	DS_ASSERT(utf32_write != NULL);
	(*utf8_read) = _utf8_read_codepoint(utf8_string, utf8_length, utf32_string);
	(*utf32_write) = ((*utf8_read) > 0 ? 1 : 0);
}

void __utf16_to_utf8(char16_t const* utf16_string, int utf16_length, int* utf16_read, char* utf8_string, int utf8_capacity, int* utf8_write) {
	DS_ASSERT(utf16_string != NULL);
	DS_ASSERT(utf16_read != NULL);
	DS_ASSERT(utf8_string != NULL);
	DS_ASSERT(utf8_write != NULL);
	char32_t codepoint = 0;
...
void __utf16_to_utf32(char16_t const* utf16_string, int utf16_length, int* utf16_read, char32_t* utf32_string, int utf32_capacity, int* utf32_write) {
	DS_ASSERT(utf16_string != NULL);
	DS_ASSERT(utf16_read != NULL);
	DS_ASSERT(utf32_string != NULL);
	DS_ASSERT(utf32_write != NULL);
...
void __utf32_to_utf8(char32_t const* utf32_string, int utf32_length, int* utf32_read, char* utf8_string, int utf8_capacity, int* utf8_write) {
	DS_ASSERT(utf32_string != NULL);
	DS_ASSERT(utf32_read != NULL);
	DS_ASSERT(utf8_string != NULL);
	DS_ASSERT(utf8_write != NULL);
...
void __utf32_to_utf16(char32_t const* utf32_string, int utf32_length, int* utf32_read, char16_t* utf16_string, int utf16_capacity, int* utf16_write) {
	DS_ASSERT(utf32_string != NULL);
	DS_ASSERT(utf32_read != NULL);
	DS_ASSERT(utf16_string != NULL);
	DS_ASSERT(utf16_write != NULL);
...
#define __convert(FROMTYPE, TOTYPE, FROMNAME, TONAME, string, length, out, capacity) ({ \
	...
	if (sizeof(FROMTYPE) == sizeof(TOTYPE)) { \
		...
		int rnn = 0, \
			...
			write = 0, \
			read = 0; \
		while ((rnn < capacity) && (lnn < length)) { \
			__##FROMNAME##_to_##TONAME(src, (length - lnn), &read, dst, (capacity - rnn), &write); \
			if (read == 0 || write == 0) \
				break; \
			src += read; \
			dst += write; \
...
int _utf8_to_utf16(char const* utf8_string, int utf8_length, char16_t* utf16_string, int utf16_capacity) {
	return __convert(char, char16_t, utf8, utf16, utf8_string, utf8_length, utf16_string, utf16_capacity);
}

int _utf8_to_utf32(char const* utf8_string, int utf8_length, char32_t* utf32_string, int utf32_capacity) {
	return __convert(char, char32_t, utf8, utf32, utf8_string, utf8_length, utf32_string, utf32_capacity);
}

int _utf8_to_ucs2(char const* utf8_string, int utf8_length, wchar_t* ucs2_string, int ucs2_capacity) {
	return __convert(char, char16_t, utf8, utf16, utf8_string, utf8_length, (char16_t*)ucs2_string, ucs2_capacity);
}

int _utf16_to_utf8(char16_t const* utf16_string, int utf16_length, char* utf8_string, int utf8_capacity) {
	return __convert(char16_t, char, utf16, utf8, utf16_string, utf16_length, utf8_string, utf8_capacity);
}

int _utf16_to_utf32(char16_t const* utf16_string, int utf16_length, char32_t* utf32_string, int utf32_capacity) {
	return __convert(char16_t, char32_t, utf16, utf32, utf16_string, utf16_length, utf32_string, utf32_capacity);
}

int _utf32_to_utf8(char32_t const* utf32_string, int utf32_length, char* utf8_string, int utf8_capacity) {
	return __convert(char32_t, char, utf32, utf8, utf32_string, utf32_length, utf8_string, utf8_capacity);
}

int _utf32_to_utf16(char32_t const* utf32_string, int utf32_length, char16_t* utf16_string, int utf16_capacity) {
	return __convert(char32_t, char16_t, utf32, utf16, utf32_string, utf32_length, utf16_string, utf16_capacity);
}

int _utf32_to_ucs2(char32_t const* utf32_string, int utf32_length, wchar_t* ucs2_string, int ucs2_capacity) {
	return __convert(char32_t, char16_t, utf32, utf16, utf32_string, utf32_length, (char16_t*)ucs2_string, ucs2_capacity);
}

int _ucs2_to_utf8(wchar_t const* ucs2_string, int ucs2_length, char* utf8_string, int utf8_capacity) {
	return __convert(char16_t, char, utf16, utf8, (char16_t*)ucs2_string, ucs2_length, utf8_string, utf8_capacity);
}